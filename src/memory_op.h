...
#define PKT_BUF_SIZE 2048
    ...
    struct DmaMemoryPair        memory_allocate_dma             (const ::VfioFd vfio_fds,size_t size, bool require_contiguous);
    struct DmaMemoryPair        memory_allocate_via_vfiomap     (const ::VfioFd vfio_fds,size_t size, bool require_contiguous);
    uint64_t                    setup_dma_ret_iova              (const ::VfioFd vfio_fds,void* virt_addr, size_t size);
