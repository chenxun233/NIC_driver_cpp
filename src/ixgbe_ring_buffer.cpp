...
bool IXGBE_RxRingBuffer::_linkDescWithPKTBuf(){
	if (_p_mem_pool == nullptr) {
	...
	for (uint32_t i = 0; i < _p_mem_pool->getNumOfBufs(); i++) {
		volatile union ixgbe_adv_rx_desc* rxd = _p_descriptors + i;
		struct pkt_buf* buf = _p_mem_pool->takeOutPktBuf();
		if (!buf) {
		...
		_v_buf_virtual_addr.push_back((void*) buf);
	}
	return true;
};
